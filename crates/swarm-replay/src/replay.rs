...
mod tests {
    ...
    fn snapshot_at_tick_reconstructs_poses() {
        ...
        assert_eq!(snap0.agent_poses[0].1, Pose { x: 1.0, y: 2.0 });

        let snap1 = snapshot_at_tick(&log, 1);
        assert_eq!(snap1.agent_poses[0].1, Pose { x: 3.0, y: 4.0 });
    }
    ...
    fn render_ascii_grid_basic() {
        let snapshot = ReplaySnapshot {
            tick: 0,
            agent_poses: vec![(AgentId::from("a0".to_owned()), Pose { x: 5.0, y: 5.0 })],
            assigned_tasks: vec![],
