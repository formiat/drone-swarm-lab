...
#[cfg(feature = "mavlink-transport")]
fn fake_execution_failure(final_status: SitlRunFinalStatus, error: &str) -> SitlExecutionFailure {
...
fn telemetry_loop_recorder_tracks_terminal_waypoint_completion() {
    ...
    let final_waypoint_task_id = recorder
        .log()
        .events
        .iter()
        .rev()
        .find_map(|event| match event {
            crate::sitl_observability::SitlEvent::WaypointReached { seq, task_id, .. }
                if *seq == 1 =>
            {
                Some(task_id.clone())
            }
...
fn telemetry_loop_recorder_logs_current_seq_only_on_change() {
    ...
    let current_seq_events: Vec<_> = recorder
        ...
        .iter()
        .filter_map(|event| match event {
            crate::sitl_observability::SitlEvent::CurrentSeqChanged { seq, task_id, .. } => {
                Some((*seq, task_id.clone()))
            }
            _ => None,
        })
