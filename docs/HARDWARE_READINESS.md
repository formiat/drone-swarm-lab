...
| Urban geo route export dry-run | Portable | M84 adds optional WGS84 `geo` on every Urban node, `coordinate_mode` reporting, direct WGS84 waypoint metadata in dry-run artifacts, Urban mission-template/perception metadata, and GeoJSON importer fixtures. This is still export/testbed evidence only, not real perception, obstacle avoidance, PX4 execution, Gazebo/HIL, or hardware readiness. |
...
| MAVLink Common compiler | Portable compiler artifact | M81 compiles `MissionCommandPlan` into transport-free `MavlinkCommonPlan` / `mavlink_common_plan.v1` dry-run artifacts with typed Common commands, explicit prelude/upload-start/postlude ordering, expected ACKs, telemetry milestones, unsupported-feature records, and stable `command_ir_hash`. This is no hardware upload, no MAVLink transport, no PX4/ArduPilot capability proof, and no flight certification. |
| MAVLink capability profiles | Portable compatibility annotation | M82 adds conservative `mavlink_common_generic`, `px4`, and `ardupilot` capability profiles, `MavlinkCompatibilityReport`, compatibility classes including `supported_with_caveats` and `unknown_until_sitl_or_hardware`, explicit `required_mode_transitions`, dry-run `--mavlink-profile`, and artifact-validator checks for profile compatibility. This is not exhaustive autopilot certification, not ArduPilot SITL evidence, not vendor SDK integration, not hardware upload, and not flight certification. |
| Primitive Real Mission Pack | Portable command-mission artifact pack | M83 adds `takeoff-hold-land`, `orbit`, and `waypoint-square` dry-run missions that compile into `MissionCommandPlan` / `MavlinkCommonPlan` artifacts with expected ACKs, telemetry milestones, timeout/abort policy, and `safety_report.passed=true`. This validates static preflight and artifact consistency only: no real flight, no connected vehicle, no PX4/ArduPilot equivalence claim, no certified flight safety, and no hardware readiness. |
| Swarm command plane | Portable supervisor-integrated coordination layer | M87 adds transport-free multi-agent command-plane contracts: roles, per-agent fanout, ownership records/handoffs, replacement/abort policies, synchronized GCS command windows, SITL replay events, full manifest/report artifacts, and strict artifact validation. This is not RF mesh, distributed consensus, physical collision avoidance, simultaneous hardware takeoff evidence, PX4/ArduPilot equivalence proof, or hardware readiness. |
| Logical swarm topologies | Portable coordination-layer topology contract | M88 adds logical centralized GCS, P2P, relay, mesh, and mothership topology contracts, deterministic command routes, blocked/degraded route evidence, transport assumptions, replay/SITL events, strict artifact validation, and portable `multi_sitl.v1` fixtures. This is not an RF mesh stack, production radio routing, physical mothership behavior, distributed consensus, or hardware readiness. |
| SITL dual-stack evidence | Portable dry-run evidence layer | M89 adds a dual-stack evidence pack that compiles the same primitive command IR for PX4 and ArduPilot profiles, records profile caveats, timeout abort policy, `not_applicable_single_agent_primitive` replacement status, and FC/safety contract summaries. This is not live ArduPilot SITL proof, not PX4/ArduPilot equivalence, not hardware upload, not certified flight safety, and not hardware readiness. |
| Hardware-entry evidence pack | Portable machine-checkable gate | M97 adds `hardware_entry_pack.v1.json` generated by `sitl_agent --hardware-entry-pack --output-dir <dir>` and validated through `artifact_validator --mode hardware-entry-pack --strict`. The pack records mission-family coverage, preflight result, FC contract result, optional primitive/Urban/swarm/dual-stack evidence, degraded policy assumptions, checklist fields, explicit evidence refs, readiness status, caveats, limitations, and blockers. Mock/local executor evidence is classified as `mock_execution_validated`; stronger statuses such as `execute_validated_locally` require selected airframe/link/autopilot, verified fence/failsafe, rehearsed abort procedure, empty blockers, and a live SITL or transport-backed evidence reference. Hardware entry without a valid M97 pack is forbidden, but a valid pack is still not hardware flight approval, certification, operator training, or a SITL=safe-hardware claim. |
...
- M82 dry-run capability profile output is reviewed for the selected stack. Any `unsupported`, `requires_stack_specific_mapping`, or `unknown_until_sitl_or_hardware` result must block hardware-facing claims unless a later explicit hardware workflow accepts it with separate evidence.
...
- For M83 primitive missions, review the dry-run artifact before any future manual upload. `takeoff-hold-land`, `orbit`, and `waypoint-square` artifacts prove command lifecycle shape only; orbit portability and landing completion remain stack-specific until later SITL/hardware evidence.
- M97 hardware-entry pack exists for the candidate mission and passes `artifact_validator --mode hardware-entry-pack --strict`. If the pack is missing, blocked, has failing preflight, has blocking FC contract violations, lacks `first_allowed_mission_type`, or requires multi-drone review without a passed single-drone gate, hardware entry is forbidden. `mock_execution_validated` is local executor evidence only; `execute_validated_locally` and `degraded_partially_evidenced` require live/transport evidence refs plus selected airframe/link/autopilot, verified fence/failsafe, and rehearsed manual abort.
