...
| Swarm command plane | Portable fake-tested coordination layer | M87 adds transport-free multi-agent command-plane contracts: roles, per-agent fanout, ownership records/handoffs, replacement/abort policies, synchronized GCS command windows, replay events, and optional manifest/report summaries. This is not RF mesh, distributed consensus, physical collision avoidance, simultaneous hardware takeoff evidence, PX4/ArduPilot equivalence proof, or hardware readiness. |
