...
| MAVLink Common compiler | Portable compiler artifact | M81 compiles `MissionCommandPlan` into transport-free `MavlinkCommonPlan` / `mavlink_common_plan.v1` dry-run artifacts with typed Common commands, expected ACKs, telemetry milestones, unsupported-feature records, and stable `command_ir_hash`. This is no hardware upload, no MAVLink transport, no PX4/ArduPilot capability proof, and no flight certification. |
