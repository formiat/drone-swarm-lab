...
| Live multi-agent PX4/SIH execute/reallocation | Experimental local SITL | M58 adds `sitl_supervisor --connection --execute`, `scenarios/sitl.multi-agent.execute.config.json`, per-agent safety/hardware gates, sequential local endpoint execution, common event log, and structured multi-agent report. M59 adds explicit `--reupload-on-failure` mission replacement after failed-agent runtime reallocation. This is not hardware readiness; real PX4/SIH failure evidence still requires a manual captured artifact. |
