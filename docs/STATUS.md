...
| M90 MAVLink Executor Boundary | Partial transport-backed execution bridge | `MavlinkPlanExecutor` now separates ACK timeout aborts from transport/adapter failures, has a standalone `mavlink_execution_artifact.v1` schema, and `artifact_validator --mode execute` validates command/caveat presence plus declared upload/start phases. With `mavlink-transport`, `MavlinkTransportAckProvider` sends compiled `MavlinkCommonPlan` command/upload/start phases through real MAVLink command and mission-upload helpers, and `sitl_agent --connection --execute` routes upload/start through this executor before the existing telemetry progress loop. FC config transport writes/reads for params and full fence upload remain incomplete; M86 is still primarily a transport-free FC contract layer. |
