...
| M88 Logical Swarm Topologies | Complete as coordination-layer topology contract | `swarm-command-plane` now records logical topology config and deterministic command-route decisions for centralized GCS, P2P peer links, relay paths, mesh partitions, and mothership dependencies. Mothership child routes are forced through the declared parent node when reachable, and P2P artifacts include explicit peer route decisions. `multi_sitl.v1` accepts optional topology configs, manifests include compact topology summaries and full route artifacts, SITL/generic replay logs emit topology configured/route selected/blocked/degraded/dependency events, and strict artifact validation checks topology consistency. This is not an RF mesh stack, not production radio routing, not physical mothership behavior, not distributed consensus, and not hardware readiness. See `docs/SWARM_TOPOLOGIES.md`. |
| M89 SITL Dual-Stack Evidence Pack | Complete as portable dry-run evidence layer | `sitl_dual_stack_evidence` emits PX4 and ArduPilot profile dry-run artifacts from the same primitive command IR and a `sitl_dual_stack_evidence_pack.v1.json` summary. The pack records shared `command_ir_hash`, profile-specific warnings, expected ACK/telemetry counts, explicit `abort_replacement` evidence, and explicit `fc_safety_contract` evidence. `artifact_validator --mode dual-stack-evidence` validates those sections and referenced dry-run artifacts. This is not live ArduPilot SITL evidence, not hardware readiness, not production failover, and not a PX4/ArduPilot equivalence proof. |
| M90 MAVLink Executor Boundary | Partial as experimental transport-backed executor boundary | `MavlinkPlanExecutor` now separates ACK timeout aborts from transport/adapter failures, has a standalone `mavlink_execution_artifact.v1` schema, and `artifact_validator --mode execute` validates command/caveat presence plus declared upload/start phases. With `mavlink-transport`, `MavlinkTransportAckProvider` sends compiled `MavlinkCommonPlan` command/upload/start phases through real MAVLink command and mission-upload helpers, honors `--no-arm`, and keeps the post-start heartbeat boundary before telemetry progress. `sitl_agent --connection --execute --run-report ...` writes a sibling `mavlink_execution_artifact.v1.json` with the real executor report. `MavlinkTransportFcConfigProvider` supports transport-backed `PARAM_REQUEST_READ` / `PARAM_SET`, but fence mission upload is intentionally blocked with typed `Unsupported` because the current generated MAVLink Common `MISSION_COUNT` / `MISSION_ITEM_INT` structs do not expose `mission_type=MAV_MISSION_TYPE_FENCE`. Plans with `geofence_prelude` abort before `MAV_CMD_DO_FENCE_ENABLE`, arm, takeoff, upload, or start. This is experimental local SITL-facing code, not hardware readiness or certified safety. |
| M96 Dual-Stack Execution Evidence | Complete as local executor evidence layer | `sitl_dual_stack_evidence --execution` emits `dual_stack_execution_evidence.v1.json` with PX4 and ArduPilot `StackExecutionRecord`s over the same `MavlinkPlanExecutor` API boundary and one shared `command_ir_hash`. Records now carry explicit execution modes: PX4 uses `local_mock_executor`, while ArduPilot uses `scripted_profile_executor`. PX4 reaches `completed` for the primitive local executor path. ArduPilot unsupported or unevidenced profile behavior is represented as `Skipped { reason: "ardupilot_..." }` steps and lifecycle `unsupported`, not panic/abort/completed. `artifact_validator --mode dual-stack-execution` checks both stack records, explicit caveats, comparison deltas, and unsupported-not-completed semantics. This is not live ArduPilot SITL proof, not transport-backed SITL proof, not hardware readiness, and not PX4/ArduPilot equivalence. |
