...
| M96 Dual-Stack Execution Evidence | Complete as local executor evidence layer | `sitl_dual_stack_evidence --execution` emits `dual_stack_execution_evidence.v1.json` with PX4 and ArduPilot `StackExecutionRecord`s over the same `MavlinkPlanExecutor` API boundary and one shared `command_ir_hash`. PX4 reaches `completed` for the primitive local executor path. ArduPilot unsupported or unevidenced profile behavior is represented as `Skipped { reason: "ardupilot_..." }` steps and lifecycle `unsupported`, not panic/abort/completed. `artifact_validator --mode dual-stack-execution` checks both stack records, explicit caveats, comparison deltas, and unsupported-not-completed semantics. This is not live ArduPilot SITL proof, not hardware readiness, and not PX4/ArduPilot equivalence. |
