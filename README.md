...
| M90 | ✅ | MAVLink Executor Boundary: `MavlinkPlanExecutor` now separates ACK timeout aborts from transport/adapter failures, `mavlink_execution_artifact.v1` plus `artifact_validator --mode execute` validate standalone execution reports, and `sitl_agent --connection --execute` uses `MavlinkTransportAckProvider` under `mavlink-transport` for compiled `MavlinkCommonPlan` command/upload/start phases before telemetry progress. `MavlinkTransportFcConfigProvider` adds transport-backed `PARAM_REQUEST_READ` / `PARAM_SET` and compiled fence upload/fence-enable plumbing. This is still experimental SITL-facing code, not hardware readiness, and fence upload remains constrained by the MAVLink Common mission fields exposed by the current `mavlink` crate |
