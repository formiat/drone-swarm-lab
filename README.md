...
| M84 | ✅ | Urban Geo Route Export + Mission Templates: Urban nodes may carry WGS84 `geo`; route export emits `coordinate_mode` and direct WGS84 waypoint metadata when every node has geo, keeps local densified export otherwise, includes Urban mission template / blocked-policy / mock-perception metadata in dry-run artifacts, adds a GeoJSON import utility fixture, and extends artifact validation for Urban coordinate/perception metadata; still no real perception, dynamic avoidance, PX4 execution, or hardware readiness |
