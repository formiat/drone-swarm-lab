...
| Swarm Command Plane | ✅ Supervisor-integrated command-plane foundation | M87 | New `swarm-command-plane` crate builds transport-free `swarm_command_plane.v1` multi-agent command-plane artifacts with roles, per-agent command fanout, ownership records, replacement/abort policy decisions, synchronized GCS operation representation, SITL replay events, strict artifact validation, and manifest/report artifact summaries. This is mission-level coordination only: no RF mesh, no distributed consensus guarantee, no physical collision avoidance, no simultaneous hardware takeoff claim |
| Logical Swarm Topologies | ✅ Supervisor-integrated topology contract | M88 | `swarm-command-plane` records logical centralized GCS, P2P, relay, mesh, and mothership topology contracts, deterministic GCS and P2P peer command routes, mothership parent-routed child commands, degraded/blocked route reasons, transport assumptions, SITL/replay topology events, strict artifact validation, and portable `multi_sitl.v1` topology fixtures. This is coordination-layer routing evidence only: no RF mesh stack, no production radio routing, no physical mothership, no consensus guarantee, and no hardware readiness claim |
...
| `swarm-command-plane` | M87/M88 mission-level multi-agent command plane: roles, per-agent command fanout, ownership records/handoffs, failure policy, synchronized GCS operation representation, logical topology contracts, deterministic command routes, and compact artifact summaries. |
...
| M90 | 🟡 | MAVLink Executor Boundary: `MavlinkPlanExecutor` now separates ACK timeout aborts from transport/adapter failures, `mavlink_execution_artifact.v1` plus `artifact_validator --mode execute` validate standalone execution reports, and `sitl_agent --connection --execute` uses `MavlinkTransportAckProvider` under `mavlink-transport` for compiled `MavlinkCommonPlan` command/upload/start phases before telemetry progress. `MavlinkTransportFcConfigProvider` supports transport-backed `PARAM_REQUEST_READ` / `PARAM_SET`, but fence mission upload is fail-safe blocked with typed `Unsupported` because the current generated MAVLink Common mission messages do not expose `mission_type=MAV_MISSION_TYPE_FENCE`; plans with `geofence_prelude` abort before fence enable, arm, takeoff, upload, or start. This is still experimental SITL-facing code, not hardware readiness |
| M96 | ✅ | Dual-Stack Execution Evidence: `sitl_dual_stack_evidence --execution` writes `dual_stack_execution_evidence.v1.json` with PX4 and ArduPilot `StackExecutionRecord`s over the same `MavlinkPlanExecutor` API boundary and one shared `command_ir_hash`; records include explicit execution modes (`local_mock_executor` for PX4 local mock evidence, `scripted_profile_executor` for ArduPilot profile evidence), and `artifact_validator --mode dual-stack-execution` checks both records, explicit caveats, comparison deltas, and rejects unsupported behavior marked as `completed`. PX4 can complete the local primitive executor path; ArduPilot incompatible/unevidenced steps are `Skipped { reason: "ardupilot_..." }` with lifecycle `unsupported`. This is local-only executor evidence, not live ArduPilot SITL proof, transport-backed SITL proof, hardware readiness, or PX4/ArduPilot equivalence |
...
| [`docs/SWARM_TOPOLOGIES.md`](docs/SWARM_TOPOLOGIES.md) | M88 logical swarm topologies: topology schema, route decisions, replay events, artifact rules, portable fixtures, and non-goals |
