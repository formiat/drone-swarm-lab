...
Ветки делятся на два уровня:

- **Ветка 0** — обязательный предварительный этап. Это не стратегический выбор,
  а технический долг, без которого ни одна из последующих веток не может стартовать
  на надёжной базе. Детальное описание — в `docs_raw/DRONE_A.15.linear.md`,
  milestones M40–M42.
...
```
Ветка 0 (обязательный ствол: M40–M42)
...
Ветка 1 (Algorithm Depth)
 └─→ Ветка 3 — Research Benchmark     (зависит от обеих: 0 и 1)

Ветка 4 — Realism v2                  (независима, осмысленна после Ветки 0)

Ветка 7 — Platform / API              (после стабилизации semantics)

Ветка 8 — New Mission                 (после Ветки 0; независима от веток 1–7)
```

---

## Ветка 0 — Algorithm Wiring & Correctness (обязательный предварительный этап)

**Статус:** не стратегическая ветка, а технический долг — обязательное условие для
всех остальных направлений.

**Суть:** устранить конкретные дефекты, которые делают текущую базу ненадёжной для
дальнейшей работы. Без этого алгоритмические изменения оптимизируют неправильную
модель, benchmark artifacts ненадёжны, диагнозы — угадывание.

**Детальное описание этого этапа** находится в `docs_raw/DRONE_A.15.linear.md`,
milestones **M40–M42**:

- **M40 — Deterministic Reporting & Benchmark Credibility**: исправить report
  identity bug в `--mission all`, добавить детерминизм-тест (jobs=1 vs jobs=N),
  зафиксировать wildfire success semantics, обновить docs.
- **M41 — Algorithmic Gap Triage**: провести MissionAdapter через runner для
  completion/scoring, исправить BatteryAwarePlanner, диагностировать SAR+CBBA
  root cause, обновить support matrix с конкретными причинами.
- **M42 — Regression Harness v3**: группировка suites (smoke/quick/experimental),
  action-oriented failure output, обновление baseline, портируемость тестов.

После закрытия M40–M42 любая из Веток 1–8 получает надёжную точку старта.

---

## Ветка 1 — Algorithm Depth

**Статус:** делать после Ветки 0.

**Суть:** новые алгоритмические возможности, которых сейчас нет. Не исправление
ошибок — развитие системы.

**Отличие от Ветки 0:** Ветка 0 подключает то, что уже реализовано, и устраняет
известные дефекты. Ветка 1 строит новую алгоритмическую функциональность поверх
исправленной базы.
...
**Статус:** самостоятельная ветка, лучше делать после Ветки 0.
...
**Статус:** делать после Веток 0 и 1.

**Суть:** превратить платформу в доказательный исследовательский артефакт.

### Почему не раньше

Benchmark содержит methodological bugs, которые закрывает Ветка 0. Ветка 1 меняет
поведение алгоритмов — 1000-seed run до неё будет устаревшим артефактом.
...
**Статус:** независима от остальных, осмыслена после Ветки 0.
...
**Статус:** делать после Ветки 0; независима от Веток 1–7.
