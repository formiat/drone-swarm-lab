...
# Current Revision Scope After Review

После ревью текущая реализация **не считается полным закрытием исходного M59**.
Принят reduced scope текущего раунда:

- реализована foundation-часть: `--reupload-on-failure`, runtime release /
  reassignment events, report/replay schema, safety re-check перед replacement,
  deterministic replacement plan for a pending survivor, portable fake-live
  coverage;
- исправлен replacement ordering: survivor unfinished tasks first, then
  recovered tasks in manifest order, without lexicographic sorting;
- добавлены portable edge tests для текущей one-shot foundation boundary;
- milestone/status docs должны показывать M59 as partial/foundation, not
  complete.

Не реализовано в текущем reduced scope и остается follow-up M59b/M60:

- stepwise `LiveAgentController` boundary вместо one-shot `run()`;
- supervisor polling multiple live agents with heartbeat/no-progress loss
  detection during an active run;
- mission replacement for an already-running survivor through
  abort/clear/upload/execute;
- fake-live lost-during-progress test over the stepwise live loop;
- real local PX4/SIH failure-injection artifact.
...
- If manual artifact or stepwise live-loop implementation cannot be captured in
  the implementation environment, M59 should be reported as partial foundation,
  not as fully done.
