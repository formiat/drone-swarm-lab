...
1. Добавить typed capability profile model в `swarm-comms`.
   ...
   - required mode transitions are first-class profile data: for example PX4 can
     describe arming/takeoff/mission-start execution mode assumptions, while
     ArduPilot can mark the same primitive mission as requiring
     stack-specific mode mapping or SITL/hardware confirmation.
...
2. Добавить compatibility pass поверх `MavlinkCommonPlan`.
   ...
   - output содержит per-command/per-item classification, aggregate result,
     required execution mode, required mode transitions, preconditions,
     mode caveats и hardware-facing block flag;
   ...
   pub fn classify_mavlink_plan_compatibility(
       ...
       for command in plan.command_prelude.iter()
          ...
          items.push(classify_command_with_mode_requirements(
              command.command,
              command.phase,
              profile,
          ));
       ...
       for item in &plan.mission_items {
          items.push(classify_mission_item_with_mode_requirements(
              item.command,
              item.frame.as_str(),
              profile,
          ));
   ...
   В `MavlinkCompatibilityReport` запланировать:
...
4. Реализовать conservative profile rules для core primitive missions.
   ...
   - mode transition support fields существуют и заполняются из profile data:
     `required_execution_mode`, `required_mode_transitions`, `preconditions`,
     `mode_caveats`. Они должны попадать в compatibility report даже когда
     command/frame support классифицирован как `supported`.
   ...
   - PX4/ArduPilot primitive mission classifications must include mission start
     mode requirements, not only command/frame support. Example: mission upload
     and `MAV_CMD_MISSION_START` should carry profile-specific execution-mode
     assumptions/caveats.
...
8. Добавить profile compatibility tests в `swarm-comms`.
   ...
   - `primitive_mission_classification_includes_mode_requirements`;
...
11. Обновить user-facing docs и docs smoke tests.
    ...
    - docs include a compatibility matrix table for Common/PX4/ArduPilot command,
      frame, mode-transition, mission-start, loiter/orbit, geofence and parameter
      differences;
    - matrix rows are rendered from or checked against static profile data via a
      unit/docs test, not hand-maintained without verification;
    ...
    - docs smoke test asserts key phrases:
      ...
      `required_mode_transitions`, `compatibility matrix`,
      `no exhaustive autopilot certification`.

12. Добавить compatibility matrix data-sync helper/test.

    Файлы:

    - `crates/swarm-comms/src/mavlink_capability_profile.rs`;
    - `docs/MAVLINK_CAPABILITY_PROFILES.md`;
    - `crates/swarm-examples/tests/sitl_docs.rs:839`.

    Материализуемый результат:

    - profile module exposes deterministic matrix rows or a renderer, for
      example `compatibility_matrix_rows()` returning rows keyed by profile and
      capability dimension;
    - `docs/MAVLINK_CAPABILITY_PROFILES.md` contains a table whose stable row
      keys match the data from `compatibility_matrix_rows()`;
    - docs/unit test fails if a profile data row is added/changed without
      updating the docs matrix.
...
- Unit tests in `crates/swarm-comms/src/mavlink_capability_profile.rs`:
  PX4/ArduPilot primitive mission classification includes
  `required_execution_mode`, `required_mode_transitions`, `preconditions` and
  `mode_caveats`.
- Unit tests in `crates/swarm-comms/src/mavlink_capability_profile.rs`:
  `compatibility_matrix_rows()` contains Common/PX4/ArduPilot rows for command
  support, frame support, mode transitions, mission start, loiter/orbit,
  geofence and parameter support.
...
- `crates/swarm-examples/tests/sitl_docs.rs`: docs matrix is checked against
  static profile data row keys, not only phrase-smoked.
...
- Mode-transition fields can become stale if represented only as caveat text.
  M82 should store them as typed profile/report data and validate their presence
  in tests.
- Docs compatibility matrix can drift from static profile data. M82 should
  expose stable matrix rows and test the docs table against them.
